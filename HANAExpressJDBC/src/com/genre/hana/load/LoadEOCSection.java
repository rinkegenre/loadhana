		...
		if (hanaConnection != null & mssqlConnection != null) {
			...
			try {
				...
				pstmt = hanaConnection.prepareStatement(
						"INSERT INTO EOC.EO_O_GTFESECTION "
						+ "(SectionGuid , DeleteFlag,CreateDate,CreateUser,ModifyDate ,ModifyUser,"
				+ "DataSourceRef,TreatyGuid ,Section,TreatyType"
				+ ",SAPChangeDate ,SectionDesc ,Layer,MainCOB,CutOffRunOff,BusinessType,UnderwritingArea,"
				+ "RunOffPeriod,PremiumAccountingMode ,LossAccountingMode,Sunset,TimeUnitSunset,ArtificialIndicator)"
				      + "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
				

				Statement mssql_stmt = mssqlConnection.createStatement();
				ResultSet mssql_resultSet = mssql_stmt.executeQuery("select * from [TTY].[EO_O_GTFESection]");
				long rowno = 0;
				int batchno = 1;
				while (mssql_resultSet.next()) {
					...
					pstmt.setDate(11, mssql_resultSet.getDate(11));
					...
					pstmt.setBigDecimal(21, mssql_resultSet.getBigDecimal(21));
